...
public class Cup extends Competition {
    ...
    public void simulateMatches() {
        ...
        for (int i = 0; i < mr.getLast().size(); i++) {
            ...
            int winsprev = 0;
            if (firstone <= mr.getFirst().size()) {
                firstone += 1;
            } else {
                winsprev = mr.getLast().get(i).getHomeTeam().dictionary.get(this).getNoWins();// MIRAMOS VICTORIAS ANTES
                                                                                              // DEL PARTIDO
            }
            mr.getLast().get(i).simulateMatch(this);
            mr.getLast().get(i).awayTeam.update(this, mr.getLast().get(i));
            mr.getLast().get(i).homeTeam.update(this, mr.getLast().get(i));

            if (winsprev == mr.getLast().get(i).getHomeTeam().dictionary.get(this).getNoWins()) {// MIRAMOS VICTORIAS
                                                                                                 // DESPUES DEL PARTIDO
                // HAS PERDIDO
